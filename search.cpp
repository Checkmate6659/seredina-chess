...
int rfp_depth = 7, rfp_margin = 107, rfp_impr = 71; //RFP parameters (rfp_impr should be positive!)
int aspi_width = 58; //aspiration window width
...
Value search(W_Board& board, int depth, Value alpha, Value beta, SearchStack* ss, Move excluded_move = Move::NO_MOVE)
{
    ...
    for (int i = 0; i < moves.size(); i++) {
        ...
        if (cur_score > alpha)
        {
            ...
            //update hist on alpha (incorrect in theory)
